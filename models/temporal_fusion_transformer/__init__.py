...
class TemporalFusionTransformer(BaseModelWithCovariates):
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        x_cat = x["encoder_cat"]  # torch.cat([x["encoder_cat"], x["decoder_cat"]], dim=1)  # concatenate in time dimension
        x_cont = x["encoder_cont"]# torch.cat([x["encoder_cont"], x["decoder_cont"]], dim=1)  # concatenate in time dimension
        ...
        lstm_output = lstm_output_encoder
        ...
        attn_output = self.post_attn_gate_norm(attn_output, attn_input)#attn_input[:, max_encoder_length:]

        output = self.pos_wise_ff(attn_output)  # [:, -1, :]

        # skip connection over temporal fusion decoder (not LSTM decoder despite the LSTM output contains
        # a skip from the variable selection network)
        output = self.pre_output_gate_norm(output, lstm_output)
        if self.n_targets > 1:  # if to use multi-target architecture
            output = [output_layer(output) for output_layer in self.output_layer]
        else:
            output = self.output_layer(output)[:, -1, :].unsqueeze(1)
            #output = self.output_layer(output)
        # otpt = output.detach().cpu().numpy().squeeze()
        # otpt_ = self.transform_output(output, target_scale=x["target_scale"]).detach().cpu().numpy().squeeze()
        # assert np.all(otpt==otpt_), 'transformation should be identity'
        # print(f'Outpout range: [{np.min(otpt)}, {np.max(otpt)}]')
    ...
    def interpret_output(
        ...
        batch_size = len(out["encoder_attention"])
        ...
        attention = torch.concat([encoder_attention], dim=-1)
