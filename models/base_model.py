...
class BaseModel(InitialParameterRepresenterMixIn, LightningModule, TupleOutputMixIn):
    ...
    def training_step(self, batch, batch_idx):
        ...
        """
        idx = [1, 10, 100, 200, 300, 400, 600]
        if batch_idx in idx :
            #print(out['prediction'].detach().cpu().numpy().squeeze())
            print(f'For day {batch_idx} positions are: {torch.bincount(torch.argmax(out["prediction"].detach().cpu(), dim=1))}')
        """
    ...
    def compute_l1(self):
        ...
        return self.hparams.l1_reg * l1_regularization
    ...
    def step(
        ...
        if self.training and len(self.hparams.monotone_constaints) > 0:
            ...
            #out["prediction"] = out["prediction"][mask].unsqueeze(1)
        ...
        for metric in self.logging_metrics:
            for idx, y_point, y_part, encoder_target in zip(
                ...
                if len(y_hat_point_detached) > 1:
                    ...
                    target_tag = ""
                self.log(
                    f"{target_tag}{self.current_stage}_{metric.torchmetric}",
                    loss_value,
                    on_step=self.training,
                    on_epoch=True,
                    batch_size=len(x["decoder_target"]),
